...
final class PBKDF2_HMAC_SHA1 {
    ...
    public static func derive(fromPassword password: Data, saltedWith salt: Data, iterating iterations: Int = 10_000, derivedKeyLength keySize: Int? = nil) throws -> Data {
        ...
        let keySize = keySize ?? SHA1.chunkSize
        ...
        guard keySize <= Int(((pow(2,32) as Double) - 1) * Double(SHA1.chunkSize)) else {
        ...
        if password.count > SHA1.chunkSize {
            password = SHA1.hash(password)
        }
        
        // Add padding
        if password.count < SHA1.chunkSize {
            password = password + Data(repeating: 0, count: SHA1.chunkSize - password.count)
        }
        ...
        let blocks = UInt32((keySize + SHA1.digestSize - 1) / SHA1.digestSize)
        var response = Data()
