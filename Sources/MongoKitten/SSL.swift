...
extension TLS.Socket: MongoTCP {
    public static func open(address hostname: String, port: UInt16, options: ClientSettings) throws -> MongoTCP {
        ...
        let config = try TLS.Config(mode: .client, certificates: .openbsd, verifyHost: !(options.sslSettings?.invalidCertificateAllowed ?? false), verifyCertificates: !(options.sslSettings?.invalidCertificateAllowed ?? false))
