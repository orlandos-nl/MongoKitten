...
internal final class GridFSWriter {
    ...
    internal init(fs: GridFSBucket, fileId: Primitive, chunkSize: Int32, buffer: ByteBuffer? = nil) {
        ...
        self.buffer = buffer ?? GridFSWriter.allocator.buffer(capacity: Int(chunkSize))
        self.length = self.buffer.readableBytes
    }
    
    func write(data: ByteBuffer) -> EventLoopFuture<Void> {
    ...
    func finalize(filename: String, metadata: Document? = nil) -> EventLoopFuture<Void> {
        ...
        return self.flush(finalize: true).flatMap {
            ...
            do {
                let encoded = try GridFSWriter.encoder.encode(file)
    ...
    private func flush(finalize: Bool = false) -> EventLoopFuture<Void> {
        ...
        do {
            ...
            nextChunkNumber += 1
            let encoded = try GridFSWriter.encoder.encode(chunk)
